...
def processData2(data):
    ...
    def checkValue(val):
        if (val > 0): # positive change -> dot or dash
            ...
            if ((withinCharacter - uncertainty) <= val <= (withinCharacter + uncertainty)) or ((betweenCharacter - uncertainty) <= val <= (betweenCharacter + uncertainty)):
                return ""
...
s = processData2(readData("dataFile.txt"))
print("***" + s + "***")
